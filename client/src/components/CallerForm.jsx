...
export function CallerForm({ initialValue, onSubmit, onCancel }) {
    // 1. Initialize state directly from initialValue if it exists
    ...
    useEffect(() => {
        if (initialValue) {
            setForm({
                ...
                dailyLeadLimit: initialValue.dailyLeadLimit || 0,
                assignedStates: initialValue.assignedStates || [],
                isActive:
                    initialValue.isActive !== undefined
                    ...
                    >
                        {LANGUAGE_OPTIONS.map((lang) => (
                            <button
                                ...
                                onClick={() =>
                                    toggleArrayValue("languages", lang)
                                }
                            >
                                {lang}
                            </button>
                    ...
                    >
                        {STATE_OPTIONS.map((state) => (
                            <button
                                ...
                                className="pill-mini"
                                style={{
                                    borderColor: form.assignedStates.includes(
                                        state,
                                    )
                                        ? "rgba(59,130,246,0.7)"
                                        : undefined,
                                    background: form.assignedStates.includes(
                                        state,
                                    )
                                        ? "rgba(37,99,235,0.3)"
                                        : undefined,
                                ...
                                onClick={() =>
                                    toggleArrayValue("assignedStates", state)
                                }
                            >
                                {state}
                            </button>
